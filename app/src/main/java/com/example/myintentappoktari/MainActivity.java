package com.example.myintentappoktari;
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.btn_move_activity) {
            ...
            moveWithDataIntent.putExtra(MoveWithDataActivity.EXTRA_NAME, "Oktari Girl");
            moveWithDataIntent.putExtra(MoveWithDataActivity.EXTRA_AGE, 19);
            startActivity(moveWithDataIntent);
        }else if (v.getId() == R.id.btn_move_activity_object) {
            Person person = new Person();
            person.setName("OKTARI Jurusan SIREG 5A");
            person.setAge(19);
            person.setEmail("09031182126028@ilkom.unsri.ac.id");
